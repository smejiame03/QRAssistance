<?php
        ...
        $resul = "SELECT PrimerNombre, SegundoNombre FROM estudiante WHERE Correo_Estudiante='$_Correo'";
        $nombres = mysqli_query($con, $resul);
        if ($row = mysqli_fetch_row($nombres)) 
        {
            $nombres = trim($row[0]." ".$row[1]);
        }
        $resul1 = "SELECT PrimerApellido, SegundoApellido FROM estudiante WHERE Correo_Estudiante='$_Correo'";
        $apellidos = mysqli_query($con, $resul1);
        if ($row = mysqli_fetch_row($apellidos)) 
        {
            $apellidos = trim($row[0]." ".$row[1]);
        }
        $resul2 = "SELECT ID FROM estudiante WHERE Correo_Estudiante='$_Correo'";
        $ID = mysqli_query($con, $resul2);
        if ($row = mysqli_fetch_row($ID)) 
        {
            $ID = trim($row[0]);
        }
        $resul3 = "SELECT Correo_Estudiante FROM estudiante WHERE Correo_Estudiante='$_Correo'";
        $Correo = mysqli_query($con, $resul3);
        if ($row = mysqli_fetch_row($Correo)) 
        {
            $Correo = trim($row[0]);
        }
        $resul4 = "SELECT Programa FROM estudiante WHERE Correo_Estudiante='$_Correo'";
        $Programa = mysqli_query($con, $resul4);
        if ($row = mysqli_fetch_row($Programa)) 
        {
            $Programa = trim($row[0]);
        }
...
<body>
    <div class="main">
        <div>
            ...
            <a href="#" class="item_btn" onclick="top.window.location.href='../Estudiante.php?Correo='+document.getElementById('variable_user').value ; return !1">Regresar</a>
        </div>
        <div class="form_boxx">
            <h2 class="title_">INFORMACIÓN ESTUDIANTE</h2>
                ...
                <table class="table_info">
                    <thead>
                        <tr>
                            ...
                            <th class="subtitle">Programa Académico</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <td class="input_echo"><input type="text" class="input_echo_text" value="<?php echo $Programa ?>" readonly></td>
                    </tbody>
                </table>
            </div>
        </div>
